...
会话登记为 `current-session` 控制台，并标记为 `attached`。这个状态表示当前会话可以作为
人工操作中的 Butler controller；它不表示 app-server 可以在新连接里重新向该 thread 发
turn，也不表示它可以作为被托管的工作 session。
...
用户主路径是 session run：Web 里点 `启动管家托管`，或 CLI 运行 `follow-session`。
这条路径会先确认目标是独立工作 session，再调用 `codex exec resume <session-id>` 恢复
目标 session，并持续推进到完成、阻塞或需要用户选择。app-server probe 失败不必然阻止
`follow-session`；真正的阻塞以 `codex exec resume` 的返回为准。

不要把“创建了 session-run”当成“已经接管”。没有 `session_run.turn` 事件或
`.codex-butler/artifacts/session-runs/*-turn-*.json` 输出时，只能说“等待第一轮执行证据”。
当前控制台、管家会话、重复 thread、与当前控制台同 thread 的记录都不能作为目标。
...
普通使用优先用网页里的 `启动管家托管` 或 CLI `follow-session`。`advance-goal`
保留给传统目标任务链。手动运行
