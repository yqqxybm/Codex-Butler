...
Codex Butler 是一个本地管家会话工作台。它的主路径是：你给目标，Butler 管家会话负责拆分、推进、调度执行会话、审查和验证；你只在管家要求确认时介入。
正常使用只有三步：

```text
打开网页 -> 交给管家目标 -> 按管家接管状态继续
```
...
但它不是可被 app-server 重新发消息的执行会话。

## 2. 交给管家目标

在“交给管家的新目标”里写你要 Butler 推进的事，例如：
...
- `交给管家推进`：推荐。管家会建立执行链，并推进到完成、明确阻塞或需要你确认的边界。
- `只生成计划`：只看执行链拆分，不实际开始。

## 3. 继续推进

目标生成后，只看“管家接管状态”。它会显示：

- 现在状态。
- 管家判断。
- 下一步该点什么。
- Butler 的执行会话链路：管家、分析、实现、审查、验证、收尾。

- 点 `让管家继续推进`：推荐。连续推进到完成、阻塞或需要人工介入。
...
正常情况下，你不需要手动理解每个底层记录。“底层执行记录”和“最近事件”默认收起，只用于排障。
普通推进优先用“管家接管状态”。遇到可恢复的执行会话交付格式问题时，点 `让管家修复并继续`；
Butler 会自动重新跑一次，不需要你到底层记录里手动点重试。

## 4. 看结果

主要看三个地方：

- “管家接管状态”：目标是否还在推进，以及下一步该点什么。
- “底层执行记录”：哪一步完成、哪一步阻塞，仅排障时展开。
...
- 验证失败：查看底层执行记录里的验证结果，再让 Butler 修复或人工处理。
