...
```text
打开网页 -> 确认推荐 session -> 接管选中的 session
```
...
在“选择 Session”里看推荐卡片。页面会自动选中最适合接管的工作 session。
...
- “状态数字”：看总数。
- “选择 Session”：检查或更换要接管的 session。
- “排障记录”：看是哪一步失败。
- “最近事件”：看 Butler 实际做过什么。

常见处理：

- 连接诊断不可达：仍可尝试接管；真正是否能推进以 `codex exec resume` 结果为准。
...
- `接管选中的 session` 会真实恢复并推进选中的 Codex session；不要在未准备好的 session 上随便启动。
