...
Codex Butler 是一个本地网页管家台，用来把“我要做什么”拆成可推进的 Codex 工作流，并在本机记录任务、验证和结果。
...
```text
输入目标 -> 生成并自动推进 -> 按当前目标卡片继续
```

日常只看“当前目标”卡片。它会显示当前状态、下一步动作和处理链路。
已有会话、内部步骤、状态数字和事件记录默认作为高级/排障信息处理；
旧会话不可达不会阻止你直接推进新目标。
...
- 阻塞诊断：网页目标卡片显示下一步；可恢复的 worker 交付问题会自动重跑一次。
...
`npm run smoke` 会检查语法、测试和 app-server 基础能力。真实 worker turn 探针会消耗模型调用，需要时手动运行：
